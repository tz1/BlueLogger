...
public class BlueLogger extends Activity {
     ...
     * classes.
     */
	public static final String LOG_TAG = "BlueLogger";
...
*/                
    ...
    private class GetAGPSThread extends Thread {
    	public GetAGPSThread() {
    	}
        public void run() {
        	mEmulatorView.write("GetAGPS\r\n".getBytes(),9);
        	...
        	// Get URL
        	try {
        		String retr = "Accessing SkyTraq FTP\r\n";
        		System.err.println(retr);
        		mEmulatorView.write(retr.getBytes(),retr.length());
        		URL eph = new URL("ftp://skytraq:skytraq@60.250.205.31/ephemeris/Eph.dat");
        		URLConnection conn = eph.openConnection();
        		InputStream in = new BufferedInputStream(conn.getInputStream());
        		try {
        			int curr;
        			while(true) {
        				curr = in.read(buf,len,bufmax-len);
                		retr = "Read: " + len + "\r\n";
                		System.err.println(retr);
                		mEmulatorView.write(retr.getBytes(),retr.length());
        				if( curr > 0 )
        			...
        			mEmulatorView.write(e.getMessage().getBytes(),e.getMessage().length());
        		}
        		finally {
        			in.close();
        		}
 
        		// Write to file, later do the comm to put via bluetooth.
        		retr = "Retrieved: " + len + "\r\n";
        		System.err.println(retr);
        		mEmulatorView.write(retr.getBytes(),retr.length());

        		File ephf0 = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS),"skytraq.eph");
        		FileOutputStream ephf = new FileOutputStream(ephf0);
        		BufferedOutputStream ephw = new BufferedOutputStream(ephf);
        		ephw.write(buf,0,len);
        		ephw.close();
        		retr = "Wrote skytraq.eph to download directory\r\n";
        		System.err.println(retr);
        		mEmulatorView.write(retr.getBytes(),retr.length());
        	}catch (Exception e){
        		System.err.println("Error: " + e.getMessage());
    ...
    private GetAGPSThread mGetAGPSThread;
    
    private class SendAGPSThread extends Thread {
    	public SendAGPSThread() {
    	}
        ...
        public void run() {
        	...
        	try {
        		...
        		File ephf0 = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS),"skytraq.eph");
        		FileInputStream ephf = new FileInputStream(ephf0);
        		BufferedInputStream ephw = new BufferedInputStream(ephf);
        		len = ephw.read(buf,0,bufmax);
        		ephw.close();
        		if( len < 80000 )
        			return;
        		...
        		retr = "Cannot access AGPS: " + e.getMessage()+"\r\n" + "Please Get the AGPS again\r\n";
        		System.err.println(retr);
        		mEmulatorView.write(retr.getBytes(),retr.length());
        	}
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case R.id.preferences:
        	...
        	return true;
        case R.id.getagps:
        	mGetAGPSThread = new GetAGPSThread();
        	mGetAGPSThread.start();
        	return true;
        case R.id.sendagps:
        	mSendAGPSThread = new SendAGPSThread();
        	mSendAGPSThread.start();
        	return true;
...
*/
    ...
    public void append(byte[] buffer, int base, int length) {
        for (int i = 0; i < length; i++) {
            ...
            try {
                if (BlueLogger.LOG_CHARACTERS_FLAG) {
                    char printableB = (char) b;
                    if (b < 32 || b > 126) {
                        printableB = ' ';
                    }
                    Log.w(BlueLogger.LOG_TAG, "'" + Character.toString(printableB)
                            + "' (" + Integer.toString(b) + ")");
                }
                process(b);
                mProcessedCharCount++;
            } catch (Exception e) {
                Log.e(BlueLogger.LOG_TAG, "Exception while processing character "
    ...
    private void selectGraphicRendition() {
        for (int i = 0; i <= mArgIndex; i++) {
            ...
            if (code == 0) { // reset
                ...
                if (BlueLogger.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
                    Log.w(BlueLogger.LOG_TAG, String.format("SGR unknown code %d", code));
                }
            }
        }
    }
    ...
    private void logError(String error) {
        if (BlueLogger.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
            Log.e(BlueLogger.LOG_TAG, error);
        }
        finishSequence();
    }
...
*/
     ...
     */
    public void initialize(BlueLogger blueLogger) {
    	mBlueTerm = blueLogger;
        mTextSize = 15;
        mForeground = BlueLogger.WHITE;
        mBackground = BlueLogger.BLACK;
        updateText();
